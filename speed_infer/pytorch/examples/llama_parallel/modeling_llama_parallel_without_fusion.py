...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        inv_freq = 1.0 / (torch.tensor(base).double() **
                          (torch.arange(0, dim, 2).double().to(device) / dim).double())
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.acl_encoder_model = torch.classes.ModelTorch.ModelTorch("llama_7b_encoder_without_fusion_model")
        self.acl_decoder_model = torch.classes.ModelTorch.ModelTorch("llama_7b_decoder_without_fusion_model")
        ...
        print(self.acl_decoder_param)
        self.acl_encoder_model.set_param(self.acl_decoder_param)
        self.acl_decoder_model.set_param(self.acl_decoder_param)
        self.acl_encoder_inputs = [None] * 5
    ...
    def forward(
        ...
        if self.weightFlag is False:
            ...
            self.acl_encoder_model.set_weight(weights_a)
        ...
        if past_key_values is None:
            ...
            self.acl_encoder_inputs = [hidden_states, position_ids, cosTable, sinTable, attention_mask]
            acl_model_out = self.acl_encoder_model.execute(self.acl_encoder_inputs, self.acl_decoder_param)
            hidden_states = acl_model_out[0]
            past_kv = ()
            tmp_kv = (acl_model_out[1].permute(0, 2, 1, 3), acl_model_out[1+self.num_layers].permute(0, 2, 1, 3))
            past_kv += (tmp_kv,)
            for i in range(self.num_layers - 1):
                tmp_kv = (acl_model_out[i+2].permute(0, 2, 1, 3), acl_model_out[i+2+self.num_layers].permute(0, 2, 1, 3))
                past_kv += (tmp_kv,)
            ...
            past_keys, past_values = map(list, zip(*past_key_values)) # (num_layers, 2)
            for i in range(len(past_keys)):
                past_keys[i] = past_keys[i].permute(0, 2, 1, 3)
                past_values[i] = past_values[i].permute(0, 2, 1, 3)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        # print("causal model init success >>>>>")
    ...
    def forward(
        ...
        ```"""
        # print("causal model preprocessing start >>>>>")
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # print("causal model preprocessing success >>>>>")
