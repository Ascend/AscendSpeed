...
MAX_SEQ_LENGTH = 2048 # 自定义最大输入输出长度，默认值2048
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.hidden_size_nz =  self.hidden_size // self.nz_dim // self.world_size
        ...
        self.acl_operation = torch.classes.ModelTorch.ModelTorch("llama_7b_flashattention_model")
        acl_param = json.dumps({"headNum": self.num_heads // self.world_size, "rmsNormEps": self.rms_norm_eps,
    ...
    def forward(
        ...
        if self.batch_num != batch_size:
            ...
            self.attention_mask_max = torch.zeros(
                (self.batch_num, math.ceil(self.max_sequence_length / 16), self.max_sequence_length, 16),
                device='npu',
                dtype=torch.half
            ).contiguous()
        ...
        if self.weightFlag is False:
            weights_a = [self.state_dict()["embed_tokens.weight"]]
            weights_p = []
            for i in range(self.num_layers):
            ...
            weights_a.append(self.state_dict()["norm.weight"])
            weights_a.append(self.lm_head_weight)

            self.acl_operation.set_weight(weights_a)
            self.weightFlag = True

        if past_key_values is None: # 首token
            ...
            self.seq_len = torch.tensor([self.token_num] * self.batch_num, dtype=torch.int32, device=self.k_cache_input.device)
            ...
            self.attention_mask_max *= 0
            if attention_mask is not None:
                attention_mask_acl = attention_mask[:,0,:,:].to(torch.half)
                padding_seq = math.ceil(self.token_num / 16) * 16 - self.token_num
                attention_mask_acl = F.pad(input=attention_mask_acl, pad=(0, padding_seq, 0, padding_seq), mode='constant', value=0)
                attention_mask_acl = attention_mask_acl.view(self.batch_num, self.token_num+padding_seq, (self.token_num+padding_seq)//16, 16).transpose(1, 2)
                self.attention_mask_max[:self.batch_num, :(self.token_num + padding_seq) // 16, :self.token_num + padding_seq, :16] += attention_mask_acl
            ...
            param = json.dumps({"tokenOffset": [self.token_num] * self.batch_num, "seqLen": [self.token_num] * self.batch_num})
            
            acl_model_out = self.acl_operation.execute(self.acl_operation_inputs, param)
            ...
            sinTable = self.sinTable
            self.token_offset = torch.full((self.batch_num,), self.token_num, dtype=torch.int32, device=self.k_cache_input.device)
            self.seq_len = torch.tensor([1] * self.batch_num, dtype=torch.int32, device=self.k_cache_input.device)
            self.acl_operation_inputs[0] = input_ids
            self.acl_operation_inputs[1] = position_ids
            self.acl_operation_inputs[2] = cosTable
            self.acl_operation_inputs[3] = sinTable
            ...
            if attention_mask is not None:
                ...
                padding_seq = math.ceil(seq_len_enc / 16) * 16 - seq_len_enc
                attention_mask_acl = F.pad(input=attention_mask_acl, pad=(0, padding_seq), mode='constant', value=0)
                attention_mask_acl = attention_mask_acl.unsqueeze(-2).repeat(1, self.nz_dim, 1).view(self.batch_num, 16, (seq_len_enc + padding_seq) // 16, 16).transpose(1,2)
                self.attention_mask_max[:self.batch_num, :(seq_len_enc + padding_seq) // 16, :16, :16] += attention_mask_acl
            ...
            param = json.dumps({"tokenOffset": [self.token_num] * self.batch_num, "seqLen": [1] * self.batch_num})
