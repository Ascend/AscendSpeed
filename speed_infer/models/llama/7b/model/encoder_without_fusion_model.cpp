...
#include "encoder_without_fusion_model.h"
...
void EncoderWithoutFusionModel::Param::FromString(const std::string &param)
{
    ...
    ATB_LOG(INFO) << "Llama7BEncoderModel param rmsNormEps:" << rmsNormEps << ", headNum:" << headNum << ", dk:" << dk
                  << ", layerNum:" << layerNum << ", rank:" << rank << ", rankSize" << rankSize;
}

EncoderWithoutFusionModel::EncoderWithoutFusionModel(const std::string &param)
    : Model("EncoderWithoutFusionModel", param)
{
    param_.FromString(param);
}

EncoderWithoutFusionModel::~EncoderWithoutFusionModel() = default;

uint32_t EncoderWithoutFusionModel::GetInputNum() const { return graph_.inTensors.size(); }

uint32_t EncoderWithoutFusionModel::GetOutputNum() const { return graph_.outTensors.size(); }

atb::Status EncoderWithoutFusionModel::InferShape(const std::vector<atb::TensorDesc> &inTensorDescs,
                                                  std::vector<atb::TensorDesc> &outTensorDescs)
{
    ATB_LOG(INFO) << "Enter EncoderWithoutFusionModel InferShape";
    ...
    ATB_LOG(INFO) << "EncoderWithoutFusionModel InferShape Looping";
...
void EncoderWithoutFusionModel::BuildGraph()
{
    ATB_LOG(INFO) << "Enter EncoderWithoutFusionModel BuildGraph";
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        atb_speed::llama_13b::EncoderParallelLayer(modelParam, &op);
