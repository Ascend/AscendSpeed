...
const int INPUT_TENSOR_COUNT_BEFORE_KEY = 6;
...
void DecoderRopeModel::Param::FromString(const std::string &param)
{
    ...
    ATB_LOG(INFO) << "Llama7BDecoderRopeModel param rmsNormEps:" << rmsNormEps << ", headNum:" << headNum
                  << ", dk:" << dk << ", layerNum:" << layerNum << ", rank:" << rank << ", rankSize:" << rankSize;
}

DecoderRopeModel::DecoderRopeModel(const std::string &param) : Model("DecoderRopeModel", param)
{
    param_.FromString(param);
}

DecoderRopeModel::~DecoderRopeModel() = default;

uint32_t DecoderRopeModel::GetInputNum() const { return graph_.inTensors.size(); }

uint32_t DecoderRopeModel::GetOutputNum() const { return graph_.outTensors.size(); }

atb::Status DecoderRopeModel::InferShape(const std::vector<atb::TensorDesc> &inTensorDescs,
                                         std::vector<atb::TensorDesc> &outTensorDescs)
{
    ATB_LOG(INFO) << "Enter DecoderRopeModel InferShape";
...
void DecoderRopeModel::BuildGraph()
{
    ATB_LOG(INFO) << "Enter DecoderRopeModel BuildGraph";
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        atb_speed::llama_13b::DecoderParallelRopeLayer(modelParam, &op);
