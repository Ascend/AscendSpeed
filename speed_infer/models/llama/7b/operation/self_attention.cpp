...
enum SelfAttentionTensorId {
    IN_MIXED_QUERY = 0, // // [bs, sq, hn, hs]
    IN_MIXED_KEY,       // // [bs, sq, hn, hs]
    IN_MIXED_VALUE,     // // [bs, sq, hn, hs]
    IN_ATTENTION_MASK,
    OUT_CONTEXT_OUT,
    OUT_PRESENT_KEY,
    OUT_PRESENT_VALUE,
...
atb::Status SelfAttention(const SelfAttentionParam &param, atb::Operation **operation)
{
    ...
    size_t nodeId = 0;
    auto &permutePresentKNode = opGraph.nodes.at(nodeId++);
    auto &permutePresentVNode = opGraph.nodes.at(nodeId++);
    ...
    atb::infer::TransposeParam oriTransposeParam = {{0, 1, 2, 3}};
    CreateOperation(oriTransposeParam, &permutePresentKNode.operation);
    permutePresentKNode.inTensorIds = {IN_MIXED_KEY};
    permutePresentKNode.outTensorIds = {OUT_PRESENT_KEY};

    CreateOperation(oriTransposeParam, &permutePresentVNode.operation);
    permutePresentVNode.inTensorIds = {IN_MIXED_VALUE};
    permutePresentVNode.outTensorIds = {OUT_PRESENT_VALUE};
    ...
    permuteKNode.inTensorIds = {IN_MIXED_KEY};
    ...
    permuteVNode.inTensorIds = {IN_MIXED_VALUE};
